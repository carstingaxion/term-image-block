...
if ( ! class_exists( 'Carstingaxion_Term_Image_Renderer' ) ) {
	 ...
	 * of taxonomy terms from the current query context.
	 *
	 * @since 0.1.0
	 */
	class Carstingaxion_Term_Image_Renderer {
		...
		public function render( array $attributes ): string {
			...
			*/
			if ( ! $taxonomy || ! $term_id ) {
				[ $taxonomy, $term_id ] = $this->auto_detect_term( $taxonomy, $term_id );
			}
		 ...
		 */
		private function auto_detect_term( string $taxonomy, int $term_id ): array {
			/*
			...
			* the current post if its post type is registered for the taxonomy.
			*/
			if ( $taxonomy && ! $term_id ) {
				$queried_object = get_queried_object();
...
echo Carstingaxion_Term_Image_Renderer::get_instance()->render( $attributes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
